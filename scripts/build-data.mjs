...
async function fetchJson(url, options = {}, timeoutMs = 30_000) {
  ...
  try {
    const res = await fetch(url, { ...options, signal: ctrl.signal, headers: { 'user-agent': 'CargoNav/2.0 (+https://github.com/LuwnFM/SCtraderoute)', ...(options.headers || {}) } })
...
function normalizeUex(commoditiesRaw, terminalsRaw, pricesRaw, vehiclesRaw, jumpRaw) {
  ...
  const ships = vehiclesRaw
    ...
    .map((v) => ({
      id: Number(v.id), name: String(v.name_full || v.name || ''), scu: Number(v.scu), manufacturer: String(v.company_name || ''), crew: String(v.crew || ''),
...
async function main() {
  ...
  console.log(`Wrote ${OUTPUT}: ${snapshot.listings.length} listings, ${snapshot.terminals.length} terminals, ${snapshot.distances.length} distances`)
  console.log(report)
}
