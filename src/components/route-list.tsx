...
import { estimateTravelTime, formatTravelDuration, formatTravelDurationRange, type TravelTimeEstimate } from '@/lib/travel-time'
...
function travelEtaText(travel: TravelTimeEstimate) {
  return travel.hasTravelRange
    ? formatTravelDurationRange(travel.totalSecondsMin, travel.totalSecondsMax)
    : formatTravelDuration(travel.totalSecondsMin)
}

function travelEtaPrefix(travel: TravelTimeEstimate) {
  if (travel.hasTravelRange) return travel.isGameData ? 'ETA полёта ~' : 'ETA β ~'
  return travel.isGameData ? 'ETA min ≥' : 'ETA β ~'
}

function travelProfitRateText(travel: TravelTimeEstimate) {
  if (travel.profitPerMinuteMin == null || travel.profitPerMinuteMax == null) return null
  if (travel.hasTravelRange) return `~${formatNumber(travel.profitPerMinuteMin)}–${formatNumber(travel.profitPerMinuteMax)} aUEC/мин`
  return `${travel.isGameData ? '≤' : '~'}${formatNumber(travel.profitPerMinuteMax)} aUEC/мин`
}
...
export function RouteList({ routes, ship, loading, error, onRetry, favorites, onToggleFavorite }: Props) {
  ...
  const detailTravel = detail ? estimateTravelTime(detail, { ship }) : null
  ...
  return <>
    <ol className="space-y-3">
      {visible.map((r, i) => {
        const eta = estimateTravelTime(r, { ship })
        const rateText = eta ? travelProfitRateText(eta) : null
        return <li key={r.key}><article className="hud-panel group relative overflow-hidden p-4 transition-colors hover:border-primary/60 sm:p-5"><div className="grid gap-4 lg:grid-cols-[auto_1fr_auto] lg:items-center">
          ...
          <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="font-display text-lg font-semibold uppercase tracking-[0.08em]">{r.commodity.name}</h3>{r.commodity.isIllegal && <Badge variant="destructive" className="font-mono text-[10px] uppercase">контрабанда</Badge>}{!r.sameSystem && <Badge variant="outline" className="font-mono text-[10px] uppercase">межсистемный</Badge>}{r.stale && <Badge variant="outline" className="border-primary/50 font-mono text-[10px] uppercase text-primary">stale</Badge>}{r.sources.map((source) => <Badge key={source} variant="secondary" className="font-mono text-[10px] uppercase">{source}</Badge>)}</div>
            ...
            <p className="mt-2 font-mono text-[11px] text-muted-foreground">{!r.sameSystem ? `${r.path.systems.join(' → ')}${r.path.jumpCount != null ? ` · ${r.path.jumpCount} переход(а)` : ''}` : `Внутри ${r.from.system}`}{eta ? ` · ${travelEtaPrefix(eta)}${travelEtaText(eta)}` : ''}</p>
          </div>
          <div className="flex flex-wrap items-end justify-between gap-4 border-t border-border pt-3 lg:w-72 lg:flex-col lg:items-end lg:justify-center lg:border-l lg:border-t-0 lg:pl-5 lg:pt-0"><div className="text-left lg:text-right"><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Чистая прибыль</p><p className="font-display text-2xl font-semibold tabular-nums text-success">+{formatNumber(r.profit)}</p><p className="font-mono text-[11px] text-muted-foreground">{formatNumber(r.units)} SCU · маржа {r.margin.toFixed(0)}%{rateText ? ` · ${rateText}` : ''}</p></div><div className="flex gap-2"><Button variant="ghost" size="icon" className="h-9 w-9 text-primary" aria-label={favorites.has(r.key) ? 'Убрать из избранного' : 'Добавить в избранное'} onClick={() => onToggleFavorite(r.key)}><Icon name="Star" size={16} fill={favorites.has(r.key) ? 'currentColor' : 'none'} /></Button><Button variant="outline" size="sm" className="font-mono text-xs uppercase tracking-[0.12em]" onClick={() => setDetail(r)}>Разбор рейса</Button></div></div>
        </div></article></li>
      })}
    </ol>
    ...
    <Dialog open={!!detail} onOpenChange={(o) => !o && setDetail(null)}><DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">{detail && <><DialogHeader><DialogTitle className="font-display uppercase tracking-[0.1em]">{detail.commodity.name}</DialogTitle><DialogDescription className="font-mono text-xs">{detail.from.name} → {detail.to.name}</DialogDescription></DialogHeader><dl className="grid grid-cols-2 gap-px border border-border bg-border font-mono text-sm sm:grid-cols-3"><Cell label="Груз" value={`${formatNumber(detail.units)} SCU`} /><Cell label="Вложения" value={`${formatNumber(detail.investment)} aUEC`} /><Cell label="Выручка" value={`${formatNumber(detail.revenue)} aUEC`} /><Cell label="Прибыль" value={`+${formatNumber(detail.profit)} aUEC`} accent /><Cell label="Прибыль за SCU" value={`${formatNumber(detail.profitPerScu)} aUEC`} /><Cell label="ROI" value={`${detail.roi.toFixed(1)} %`} /><Cell label="Supply" value={detail.supply == null ? 'нет данных' : `${formatNumber(detail.supply)} SCU`} /><Cell label="Demand" value={detail.demand == null ? 'нет данных' : `${formatNumber(detail.demand)} SCU`} /><Cell label="Ограничивает" value={detail.limitedBy || '—'} /><Cell label="Цена покупки" value={formatAge(detail.buyUpdatedAt)} /><Cell label="Цена продажи" value={formatAge(detail.sellUpdatedAt)} /><Cell label="Расстояние" value={detail.distanceGm == null ? 'нет данных' : `${formatNumber(detail.distanceGm, 1)} Gm`} /><Cell label={detailTravel?.hasTravelRange ? 'Полёт, диапазон' : detailTravel?.isGameData ? 'Полёт, минимум' : 'ETA β'} value={detailTravel ? `${detailTravel.hasTravelRange ? '~' : detailTravel.isGameData ? '≥' : '~'}${travelEtaText(detailTravel)}` : 'нет данных'} /><Cell label={detailTravel?.hasTravelRange ? 'Прибыль/мин, диапазон' : detailTravel?.isGameData ? 'Прибыль/мин, максимум' : 'Прибыль/мин β'} value={detailTravel ? (travelProfitRateText(detailTravel) || 'нет данных') : 'нет данных'} /></dl><div className="space-y-1 font-mono text-xs text-muted-foreground"><p>{detail.sameSystem ? `Внутрисистемный маршрут: ${detail.from.system}` : `Путь: ${detail.path.systems.join(' → ')}${detail.path.jumpCount != null ? ` · переходов: ${detail.path.jumpCount}` : ''}`}</p><p>Источники цен: {detail.sources.length ? detail.sources.join(' + ') : 'не указаны'}.</p>{detail.availabilityUnknown && <p className="text-primary">Supply/demand известны не полностью — учитывайте это перед крупной сделкой.</p>}</div><TravelBreakdown travel={detailTravel} ship={ship} /><div className="border-t border-border pt-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-display uppercase tracking-[0.08em]">История цен</h3><p className="font-mono text-[11px] text-muted-foreground">Реальные точки UEX из same-origin кэша CargoNav; линия только соединяет реальные снимки цен между собой.</p></div><Button variant="outline" size="sm" disabled={historyLoading || !(detail.commodity.uexId && (detail.from.uexId || detail.to.uexId))} onClick={loadHistory} className="font-mono text-xs uppercase">{historyLoading ? 'Загрузка…' : 'История цены'}</Button></div>{historyError && <p className="mt-3 font-mono text-xs text-destructive">{historyError}</p>}{historyNotice && <p className="mt-3 font-mono text-xs text-primary">{historyNotice}</p>}{history && <HistoryChart points={history} />}{!(detail.commodity.uexId && (detail.from.uexId || detail.to.uexId)) && <p className="mt-3 font-mono text-xs text-muted-foreground">Для этой точки нет UEX ID, поэтому UEX history недоступна.</p>}</div></>}</DialogContent></Dialog>
  </>
}

function TravelBreakdown({ travel, ship }: { travel: TravelTimeEstimate | null; ship?: Ship | null }) {
  if (!travel) return <div className="border-t border-border pt-4"><h3 className="font-display uppercase tracking-[0.08em]">Время рейса</h3><p className="mt-2 font-mono text-xs text-muted-foreground">Для этой пары UEX пока не отдал расстояние, поэтому расчёт времени не выполняется.</p></div>

  const fuel = travel.fuelConsumptionScuPerGm != null ? travel.fuelConsumptionScuPerGm * travel.distanceGm : null
  const jumpRangeText = travel.jumpCount > 0 ? formatTravelDurationRange(travel.jumpSecondsMin, travel.jumpSecondsMax) : null

  if (!travel.isGameData) return <div className="border-t border-border pt-4">
    <div className="flex flex-wrap items-center gap-2"><h3 className="font-display uppercase tracking-[0.08em]">Время рейса</h3><Badge variant="outline" className="font-mono text-[10px] uppercase">beta fallback</Badge>{travel.hasTravelRange && <Badge variant="outline" className="font-mono text-[10px] uppercase">jump range</Badge>}</div>
    <p className="mt-2 font-mono text-xs text-muted-foreground">{ship ? `Для ${ship.name} точный stock QuantumDrive профиль ещё не получен из игровых данных.` : 'Выбран свой объём трюма без конкретного корабля.'} Quantum-сегмент пока использует старую калибровку расстояния UEX по доступным примерам SC Trade Tools. {travel.jumpCount > 0 ? 'Jump tunnel уже включён отдельным диапазоном, а не фиксированным таймером.' : ''}</p>
    {travel.jumpCount > 0 && <dl className="mt-3 grid grid-cols-2 gap-px border border-border bg-border font-mono text-xs sm:grid-cols-3"><TravelCell label="Quantum β" value={formatTravelDuration(travel.cruiseSeconds)} /><TravelCell label="Jump tunnel" value={`~${jumpRangeText}`} note={`${travel.jumpCount} переход(а)`} /><TravelCell label="Полёт, диапазон" value={`~${travelEtaText(travel)}`} /></dl>}
    <p className="mt-3 font-mono text-[11px] leading-relaxed text-primary/90">Это не полный door-to-door таймер: {travel.unknownSegments.join(', ')} и ожидание терминалов остаются сверху. Поэтому диапазон прибыли/мин не является гарантированным.</p>
    {travel.jumpCount > 0 && <div className="mt-2 font-mono text-[10px] text-muted-foreground">Источник диапазона jump tunnel: {travel.jumpTimingSource || 'operational-observation'}{travel.jumpTimingFallbackCount > 0 ? ' · fallback для старого snapshot' : ''}</div>}
  </div>

  return <div className="border-t border-border pt-4">
    <div className="flex flex-wrap items-center gap-2"><h3 className="font-display uppercase tracking-[0.08em]">Время рейса</h3><Badge variant="outline" className="border-success/50 font-mono text-[10px] uppercase text-success">game data</Badge><Badge variant="outline" className="font-mono text-[10px] uppercase">UEX distance</Badge>{travel.hasTravelRange && <Badge variant="outline" className="font-mono text-[10px] uppercase">jump range</Badge>}</div>
    <p className="mt-2 font-mono text-xs text-muted-foreground">{travel.shipName || ship?.name || 'Корабль'}{travel.driveName ? ` · штатный QD ${travel.driveName}` : ' · штатный QuantumDrive'}{travel.sourceVersion ? ` · ${travel.sourceVersion}` : ''}</p>
    <dl className="mt-3 grid grid-cols-2 gap-px border border-border bg-border font-mono text-xs sm:grid-cols-3">
      <TravelCell label="Quantum travel" value={formatTravelDuration(travel.cruiseSeconds)} />
      <TravelCell label="Spool-up" value={formatTravelDuration(travel.spoolSeconds)} />
      <TravelCell label="Calibration" value={formatTravelDuration(travel.calibrationSeconds)} />
      <TravelCell label="Cooldown после QT" value={formatTravelDuration(travel.cooldownSeconds)} note="не включён в ETA" />
      <TravelCell label="Jump tunnel" value={travel.jumpCount > 0 ? `~${jumpRangeText}` : 'нет'} note={travel.jumpCount > 0 ? `${travel.jumpCount} переход(а)` : undefined} />
      <TravelCell label="Loading / unloading" value="не учтено" />
    </dl>
    <p className="mt-3 font-mono text-[11px] leading-relaxed text-primary/90">{travel.hasTravelRange ? <>Из известных полётных компонентов диапазон составляет ~{travelEtaText(travel)}. Jump tunnel не считается фиксированным: для каждого перехода используется наблюдаемый диапазон. </> : <>Из известных компонентов минимум составляет ≥{formatTravelDuration(travel.totalSecondsMin)}. </>}Это не полный door-to-door таймер: {travel.unknownSegments.join(', ')} остаются сверху. Поэтому прибыль/мин показана без этого дополнительного overhead и не является гарантированным значением.</p>
    <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 font-mono text-[10px] text-muted-foreground"><span>Источник QD: {travel.source}</span>{travel.travelTime10GmSeconds != null && <span>10 Gm: {formatTravelDuration(travel.travelTime10GmSeconds)}</span>}{travel.quantumSpeedMps != null && <span>max QT: {formatNumber(travel.quantumSpeedMps / 1_000_000, 1)} Mm/s</span>}{fuel != null && <span>QFuel по дистанции: ~{formatNumber(fuel, 3)} SCU</span>}{travel.jumpCount > 0 && <span>Jump range: {travel.jumpTimingSource || 'operational-observation'}{travel.jumpTimingFallbackCount > 0 ? ' · fallback' : ''}</span>}</div>
  </div>
}

function TravelCell({ label, value, note }: { label: string; value: string; note?: string }) { return <div className="bg-card p-3"><dt className="uppercase tracking-[0.14em] text-muted-foreground">{label}</dt><dd className="mt-1 text-foreground">{value}</dd>{note && <dd className="mt-1 text-[10px] text-muted-foreground">{note}</dd>}</div> }
