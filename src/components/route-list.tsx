...
type HistoryPoint = { t: number; v: number; kind: 'buy' | 'sell' }
...
export function RouteList({ routes, loading, error, onRetry, favorites, onToggleFavorite }: Props) {
  ...
  useEffect(() => { setHistory(null); setHistoryError(null); setHistoryLoading(false) }, [detail])
  ...
  const loadHistory = async () => {
    ...
    try {
      const [buy, sell] = await Promise.all([fetchHistory(detail.from.uexId, detail.commodity.uexId), fetchHistory(detail.to.uexId, detail.commodity.uexId)])
      const points: HistoryPoint[] = []
      for (const row of buy) if (Number(row.price_buy) > 0 && Number(row.date_added) > 0) points.push({ t: Number(row.date_added), v: Number(row.price_buy), kind: 'buy' })
      for (const row of sell) if (Number(row.price_sell) > 0 && Number(row.date_added) > 0) points.push({ t: Number(row.date_added), v: Number(row.price_sell), kind: 'sell' })
  ...
  return <>
    <ol className="space-y-3">
      {visible.map((r, i) => <li key={r.key}><article className="hud-panel group relative overflow-hidden p-4 transition-colors hover:border-primary/60 sm:p-5"><div className="grid gap-4 lg:grid-cols-[auto_1fr_auto] lg:items-center">
        ...
        <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="font-display text-lg font-semibold uppercase tracking-[0.08em]">{r.commodity.name}</h3>{r.commodity.isIllegal && <Badge variant="destructive" className="font-mono text-[10px] uppercase">контрабанда</Badge>}{!r.sameSystem && <Badge variant="outline" className="font-mono text-[10px] uppercase">межсистемный</Badge>}{r.stale && <Badge variant="outline" className="border-primary/50 font-mono text-[10px] uppercase text-primary">stale</Badge>}{r.sources.map((source) => <Badge key={source} variant="secondary" className="font-mono text-[10px] uppercase">{source}</Badge>)}</div>
          ...
          {!r.sameSystem && <p className="mt-2 font-mono text-[11px] text-muted-foreground">{r.path.systems.join(' → ')}{r.path.jumpCount != null ? ` · ${r.path.jumpCount} переход(а)` : ''}</p>}
        </div>
        <div className="flex flex-wrap items-end justify-between gap-4 border-t border-border pt-3 lg:w-72 lg:flex-col lg:items-end lg:justify-center lg:border-l lg:border-t-0 lg:pl-5 lg:pt-0"><div className="text-left lg:text-right"><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Чистая прибыль</p><p className="font-display text-2xl font-semibold tabular-nums text-success">+{formatNumber(r.profit)}</p><p className="font-mono text-[11px] text-muted-foreground">{formatNumber(r.units)} SCU · маржа {r.margin.toFixed(0)}%</p></div><div className="flex gap-2"><Button variant="ghost" size="icon" className="h-9 w-9 text-primary" aria-label={favorites.has(r.key) ? 'Убрать из избранного' : 'Добавить в избранное'} onClick={() => onToggleFavorite(r.key)}><Icon name="Star" size={16} fill={favorites.has(r.key) ? 'currentColor' : 'none'} /></Button><Button variant="outline" size="sm" className="font-mono text-xs uppercase tracking-[0.12em]" onClick={() => setDetail(r)}>Разбор рейса</Button></div></div>
      </div></article></li>)}
    </ol>
