...
import { estimateTravelTime, formatTravelDuration, type TravelTimeEstimate } from '@/lib/travel-time'
      ...
      {visible.map((r, i) => {
        ...
        return <li key={r.key}><article className="hud-panel group relative overflow-hidden p-4 transition-colors hover:border-primary/60 sm:p-5"><div className="grid gap-4 lg:grid-cols-[auto_1fr_auto] lg:items-center">
          ...
          <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="font-display text-lg font-semibold uppercase tracking-[0.08em]">{r.commodity.name}</h3>{r.commodity.isIllegal && <Badge variant="destructive" className="font-mono text-[10px] uppercase">контрабанда</Badge>}{!r.sameSystem && <Badge variant="outline" className="font-mono text-[10px] uppercase">межсистемный</Badge>}{r.stale && <Badge variant="outline" className="border-primary/50 font-mono text-[10px] uppercase text-primary">stale</Badge>}{r.sources.map((source) => <Badge key={source} variant="secondary" className="font-mono text-[10px] uppercase">{source}</Badge>)}</div>
            ...
            <p className="mt-2 font-mono text-[11px] text-muted-foreground">{!r.sameSystem ? `${r.path.systems.join(' → ')}${r.path.jumpCount != null ? ` · ${r.path.jumpCount} переход(а)` : ''}` : `Внутри ${r.from.system}`}{eta ? ` · ${eta.isGameData ? 'ETA min ≥' : 'ETA β ~'}${formatTravelDuration(eta.totalSeconds)}` : ''}</p>
          </div>
          <div className="flex flex-wrap items-end justify-between gap-4 border-t border-border pt-3 lg:w-72 lg:flex-col lg:items-end lg:justify-center lg:border-l lg:border-t-0 lg:pl-5 lg:pt-0"><div className="text-left lg:text-right"><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Чистая прибыль</p><p className="font-display text-2xl font-semibold tabular-nums text-success">+{formatNumber(r.profit)}</p><p className="font-mono text-[11px] text-muted-foreground">{formatNumber(r.units)} SCU · маржа {r.margin.toFixed(0)}%{eta?.profitPerMinute != null ? ` · ${eta.isGameData ? '≤' : '~'}${formatNumber(eta.profitPerMinute)} aUEC/мин` : ''}</p></div><div className="flex gap-2"><Button variant="ghost" size="icon" className="h-9 w-9 text-primary" aria-label={favorites.has(r.key) ? 'Убрать из избранного' : 'Добавить в избранное'} onClick={() => onToggleFavorite(r.key)}><Icon name="Star" size={16} fill={favorites.has(r.key) ? 'currentColor' : 'none'} /></Button><Button variant="outline" size="sm" className="font-mono text-xs uppercase tracking-[0.12em]" onClick={() => setDetail(r)}>Разбор рейса</Button></div></div>
        </div></article></li>
      })}
    </ol>
...
function TravelBreakdown({ travel, ship }: { travel: TravelTimeEstimate | null; ship?: Ship | null }) {
  ...
  if (!travel.isGameData) return <div className="border-t border-border pt-4"><div className="flex flex-wrap items-center gap-2"><h3 className="font-display uppercase tracking-[0.08em]">Время рейса</h3><Badge variant="outline" className="font-mono text-[10px] uppercase">beta fallback</Badge></div><p className="mt-2 font-mono text-xs text-muted-foreground">{ship ? `Для ${ship.name} точный stock QuantumDrive профиль ещё не получен из игровых данных.` : 'Выбран свой объём трюма без конкретного корабля.'} Пока используется старая калибровка расстояния UEX по доступным примерам SC Trade Tools. Погрузка, ожидание, атмосферные участки и jump-point transit в неё не входят.</p></div>

  return <div className="border-t border-border pt-4">
    <div className="flex flex-wrap items-center gap-2"><h3 className="font-display uppercase tracking-[0.08em]">Время рейса</h3><Badge variant="outline" className="border-success/50 font-mono text-[10px] uppercase text-success">game data</Badge><Badge variant="outline" className="font-mono text-[10px] uppercase">UEX distance</Badge></div>
    ...
    <dl className="mt-3 grid grid-cols-2 gap-px border border-border bg-border font-mono text-xs sm:grid-cols-3">
      ...
      <TravelCell label="Jump points" value={travel.jumpCount > 0 ? `${travel.jumpCount} · время неизвестно` : 'нет'} />
      <TravelCell label="Loading / unloading" value="не учтено" />
    </dl>
    <p className="mt-3 font-mono text-[11px] leading-relaxed text-primary/90">Из известных компонентов минимум составляет ≥{formatTravelDuration(travel.totalSeconds)}. Это не полный door-to-door таймер: {travel.unknownSegments.join(', ')} остаются сверху. Поэтому прибыль/мин показана как верхняя граница, а не как гарантированное значение.</p>
    <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 font-mono text-[10px] text-muted-foreground"><span>Источник QD: {travel.source}</span>{travel.travelTime10GmSeconds != null && <span>10 Gm: {formatTravelDuration(travel.travelTime10GmSeconds)}</span>}{travel.quantumSpeedMps != null && <span>max QT: {formatNumber(travel.quantumSpeedMps / 1_000_000, 1)} Mm/s</span>}{fuel != null && <span>QFuel по дистанции: ~{formatNumber(fuel, 3)} SCU</span>}</div>
  </div>
}
